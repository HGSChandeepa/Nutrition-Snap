...
export default function Analytics() {
  ...
  const getWeeklyTrends = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const totalProtein = dayMeals.reduce(
        (sum, meal) => sum + meal.totalNutrition.protein,
        0
      );
      const totalCarbs = dayMeals.reduce(
        (sum, meal) => sum + meal.totalNutrition.carbs,
        0
      );
      const totalFats = dayMeals.reduce(
        (sum, meal) => sum + meal.totalNutrition.fats,
        0
      );
  ...
  if (loading) {
    return (
      <DashboardLayout
        title="Analytics"
        subtitle="Loading your detailed insights..."
      >
  ...
  return (
    <DashboardLayout
      title="Analytics"
      subtitle="Detailed insights into your nutrition patterns"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">of calorie goal</p>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="p-4 bg-green-50 rounded-lg">
                ...
                <p className="text-sm text-green-700">
                  You're most consistent with logging{" "}
                  {mealTypeData[0]?.name || "meals"} - keep it up!
                </p>
              </div>
              <div className="p-4 bg-blue-50 rounded-lg">
                ...
                <p className="text-sm text-blue-700">
                  ...
                  {topFoods[0]?.name || "not determined yet"} - logged{" "}
                  {topFoods[0]?.count || 0} times.
                </p>
              </div>
              <div className="p-4 bg-purple-50 rounded-lg">
                <h4 className="font-medium text-purple-800 mb-2">⚡ Energy</h4>
                <p className="text-sm text-purple-700">
                  Your average meal contains {stats.avgCaloriesPerMeal} calories
